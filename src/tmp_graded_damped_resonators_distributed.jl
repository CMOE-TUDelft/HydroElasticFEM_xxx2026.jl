...
@everywhere begin
  ...
  Kᵣ_vec = Mᵣ .* ωᵣ_vec .^ 2
  ζ = 0.05  # damping ratio for the resonators
  Cᵣ_vec = 2 .* ζ .* sqrt.(Kᵣ_vec .* Mᵣ)  # damping coefficients for the resonators
  ...
  function main(n::Int, ωinc::Real, vtk_output::Bool=false)
    ...
    resonators = P.resonator_array(nᵣ, fill(Mᵣ, nᵣ), Kᵣ_vec, Cᵣ_vec; delta_domain_symbols=[Symbol("δ$i") for i in 1:nᵣ], ρw=ρw, fe=PH.FESpaceConfig(space_type=P.VectorValue{1,Float64}, vector_type=Vector{ComplexF64}))
    ...
    if vtk_output
      writevtk(trians[:Ω], "tmp_vol_graded_damped_$ωinc.vtu", cellfields=["ϕ" => real(ϕₕ),"ϕin"=>x->real(inc_wave.ϕin(x))])
      writevtk(trians[:Γκ], "tmp_surf_graded_damped_$ωinc.vtu", cellfields=["κ" => abs(κₕ),"ηin" => x->abs(inc_wave.ηin(x))],nsubcells=4)
      writevtk(trians[:Γη], "tmp_struct_graded_damped_$ωinc.vtu", cellfields=["η" => abs(ηₕ)],nsubcells=4)
    end
...
savefig(plt3, "R_amplitude_graded_damped_fig9.png")
...
savefig(plt4, "eta_field_vs_freq_graded_damped_fig10.png")